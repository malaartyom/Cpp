...
class AVL {
    ...
    struct TreeNode {
        ...
        TreeNode(T x): val(x), left(nullptr), right(nullptr), children(0), h(0) {}

        TreeNode(const TreeNode& other) = default;

        TreeNode(T x, TreeNode* left, TreeNode* right, int children, int height): val(x), left(left), 
    ...
    AVL(T val): root(new TreeNode(val)) {}
    AVL(const AVL& other) {
